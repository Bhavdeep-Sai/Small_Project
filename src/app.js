...
function generateRandomScore() {
  const randomNum = Math.floor(Math.random() * possibleOutcomes.length);
  return possibleOutcomes[randomNum];
}
...
function updateScore() {
  ...
  // write conditions to update wickets/ score number if the random score is a 'W'
...
function resetGame() {
  ...
  ballElements.forEach((ball) => (ball.textContent = ''));
